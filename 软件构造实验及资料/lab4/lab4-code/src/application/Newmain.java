	...
	public static void startAtomStructureSystem() throws IOException, FileException {
		...
		while (choose1 != 0) {
			...
			case 2:
				...
				break;
			case 3:
				System.out.println("intput track index");
				int a3 = in.nextInt();
				atomStructure.deleteTrack(a3 - 1);
				break;
			case 4:
				System.out.println("input track index and object number");
				int a4 = in.nextInt();
				int b4 = in.nextInt();
				for (int i = 0; i < b4; i++) {
			...
			case 5:
				...
				System.out.println("input track a and track b(a -> b)");
...
//			    apIs.getDifference(atomStructure, atomStructure2);
	...
	public static void startStellarySystem() throws IOException, TrackException, FileException {
		StellarSystem stellarSystem = new StellarSystem()
				.buildStellarSystemFromFile("src/testtxtfiles/AtomicStructure.txt");
		System.out.println(
				"1.计算行星之间距离\n" + "2.计算行星与恒星之间的距离\n" + "3.计算某一时刻行星的位置\n" + "4.熵值\n" + "5.删除轨道\n" + "7.可视化\n" + "0.结束\n");
		int choose1 = 0;
		while (choose1 != 0) {
...
//				System.out.println(stellarSystem.getObjectLists(track).getObjectNum(stellarSystem.geTracks().get(0)));// 验证存在与否
